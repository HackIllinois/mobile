...
export default function EventScreen() {
  ...
  const renderEvent = ({ item, index }: { item: any; index: number }) => {
    ...
    return (
      <View style={{ marginBottom: 40 }}>
        {showTime && <Text style={styles.mentorTimeHeader}>{timeHeaderText}</Text>}

        {/* date on each card when no specific day selected */}
        {showDateEverywhere && <Text style={styles.cardDateLabel}>{formatDateShort(m.startTime)}</Text>}

        <TouchableOpacity
          activeOpacity={0.85}
          onPress={() => {
            setSelectedMentorSession(m);
        ...
        >
          <View style={styles.mentorCard}>
            ...
            <Text style={styles.mentorMore}>See more...</Text>
          </View>
        </TouchableOpacity>
      </View>
    );
      ...
      return (
        <View style={styles.emptyContainer}>
          <Text style={styles.emptyText}>
            No {isMentors ? 'mentorship sessions' : 'events'} scheduled {selectedDay !== null ? 'for this day' : ''}.
          </Text>
        </View>
      );
    }
